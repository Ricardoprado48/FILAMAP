...
**Versão:** 1.1  
**Tipo de documento:** Product Journey + Technical Architecture  
**Produto:** Filamap  
...
> **⚠️ Nota v1.1 — caminho do arquivo confirmado, nomes de campo ainda não.**
> Confirmamos de forma independente (biblioteca `lib3mf-core` e documentação
> técnica de terceiros) que `Metadata/slice_info.config` existe mesmo — o
> `.gcode.3mf` é um ZIP comum, e esse arquivo fica lá dentro com peso
> estimado e estatísticas por filamento. O que ainda **não está confirmado
> com um arquivo real nosso** são os nomes exatos de campo — o exemplo de
> G-code puro que inspecionamos trazia só um total combinado
> (`total filament weight [g]`), sem quebra por cor nesse arquivo
> específico. Os nomes `used_m`/`used_g`/`used_for_support`/`used_for_object`
> citados aqui podem vir direto do código-fonte do Bambu Studio — se for
> assim, são a fonte mais confiável que temos. Ainda assim, antes de
> escrever o parser definitivo, vale extrair um `slice_info.config` real (os
> scripts de diagnóstico `explore-ftp.ts` + `download-and-inspect.ts` fazem
> exatamente isso) e comparar com esses nomes de campo antes de assumir que
> batem.
...
# 62. Estado Real de Implementação (v1.1)
...
- RLS ausente nas 4 tabelas + anon key exposta no frontend — qualquer
  visitante lia/editava dados de qualquer utilizador. Corrigido.
...
- Caminho exato do `.gcode.3mf` no servidor FTPS da impressora (porta
  suspeita: 990, TLS implícito) — não confirmado com teste real ainda.
- Estrutura exata (nomes de campo) de `Metadata/slice_info.config` — ver
  nota na Seção 23.
