...
export default function App() {
  ...
  const [calcSubTab, setCalcSubTab] = useState<"calculator" | "catalog">("catalog");
  ...
  async function loadData() {
    ...
    const { data: catData } = await supabase.from("catalog_items").select("*").order("name", { ascending: true });
  ...
  // CÁLCULOS MATEMÁTICOS DE ORÇAMENTO
  ...
  async function handleSaveToCatalog() {
    ...
    if (error) {
      alert("Erro ao salvar peça no catálogo: " + error.message);
    } else {
      alert(`✅ Peça "${calcPartName.toUpperCase()}" salva com sucesso na Biblioteca!`);
      await loadData();
  ...
  async function handleDeleteCatalogItem(e: React.MouseEvent, id: string, name: string) {
    e.stopPropagation();
    if (!window.confirm(`Excluir a peça "${name}" da biblioteca?`)) return;
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "16px", minHeight: "100vh", boxSizing: "border-box" }}>
      ...
      <header style={{ borderBottom: "1px solid #334155", paddingBottom: 14, marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <h1 style={{ margin: 0, fontSize: 22, color: "#38bdf8", fontWeight: 900 }}>FILAMAP</h1>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={() => setActiveTab("ams")} style={{ flex: "1 1 120px", padding: 10, borderRadius: 8, border: "none", fontWeight: 700, fontSize: 13, cursor: "pointer", background: activeTab === "ams" ? "#0284c7" : "#1e293b", color: activeTab === "ams" ? "#fff" : "#94a3b8" }}>
            🖨️ Monitor AMS
          </button>
          <button onClick={() => setActiveTab("inventory")} style={{ flex: "1 1 120px", padding: 10, borderRadius: 8, border: "none", fontWeight: 700, fontSize: 13, cursor: "pointer", background: activeTab === "inventory" ? "#0284c7" : "#1e293b", color: activeTab === "inventory" ? "#fff" : "#94a3b8" }}>
            📦 Almoxarifado ({inventory.length})
          </button>
          <button onClick={() => setActiveTab("calc")} style={{ flex: "1 1 120px", padding: 10, borderRadius: 8, border: "none", fontWeight: 700, fontSize: 13, cursor: "pointer", background: activeTab === "calc" ? "#0284c7" : "#1e293b", color: activeTab === "calc" ? "#fff" : "#94a3b8" }}>
            🧮 Orçamento & Catálogo
          </button>
          <button onClick={() => setActiveTab("writer")} style={{ flex: "1 1 120px", padding: 10, borderRadius: 8, border: "none", fontWeight: 700, fontSize: 13, cursor: "pointer", background: activeTab === "writer" ? "#0284c7" : "#1e293b", color: activeTab === "writer" ? "#fff" : "#94a3b8" }}>
            🏷️ Gravar Tag
          </button>
        </div>
      </header>
      ...
      {/* ABA 3: ORÇAMENTO & BIBLIOTECA DE PEÇAS */}
      {activeTab === "calc" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {/* Sub-Navegador entre Calculadora e Catálogo */}
          ...
          {/* SUB-ABA 1: BIBLIOTECA DE PEÇAS */}
          {calcSubTab === "catalog" && (
            <div style={{ background: "#1e293b", padding: 16, borderRadius: 12, border: "1px solid #334155" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
                <div>
                  <h2 style={{ fontSize: 16, color: "#f8fafc", margin: 0 }}>Catálogo de Peças Orçadas</h2>
                  <p style={{ color: "#94a3b8", fontSize: 11, margin: "2px 0 0" }}>Consulte preços instantaneamente quando o cliente pedir cotação</p>
                </div>
                <input
                  type="text"
                  value={catalogSearch}
          ...
          {/* SUB-ABA 2: CALCULADORA DE PRECIFICAÇÃO */}
              ...
              <div style={{ background: "#1e293b", padding: 14, borderRadius: 12, border: "1px solid #334155" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <button
                    ...
                    style={{ background: showConfigPanel ? "#0284c7" : "#0f172a", color: "#38bdf8", border: "1px solid #38bdf8", padding: "5px 10px", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer" }}
                  >
